...
- 3030 的 Web、Worker 和 mock generation 统一改用唯一镜像 `goodgood:gg026-local`，镜像 ID 为
  `sha256:88610cb4775124f5bfe9b0c4bc9f14d0526180329f7704d658d48f7010f6eddb`；PostgreSQL、Valkey 与
  对象存储未重建，全部容器健康。
...
- 浏览器验证 `/create`、`/credits` 与 `/admin/users`：`帮助` 下方积分入口、用户名下方单枚
  ...
  取消，未执行账户写操作。
- 验收反馈修正：赠送测试积分的 100/500/1000 预设增加明确 `aria-pressed` 状态；当前选项固定为
  宫墙红底、白字，悬停为深红，未选项保持白底灰边。定向测试 13/13 通过；实机从 100 切换到 500
  后，视觉与无障碍选中状态同步迁移，未提交赠送操作。
- 反馈后的单次完整门禁构建、lint、类型检查均通过；260 项测试中 253 通过、6 个 opt-in 跳过，
  仅既有 O1Key adapter 重启恢复用例因轮询超时失败。该失败用例随后单独复跑 1/1 通过，确认是与
  本次 UI 改动无关的时序抖动；按仓库约定未重复整套门禁。
