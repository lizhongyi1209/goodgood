...
- 用户报告另一窗口遵循f68ba81却打开无视频的早期页面。只读确认该提交含视频，旧gg027-local Web仍在3030，当前Node从dist加载且无Git产物来源强制校验。
- 缺少那次URL/启动命令，尚未证明事件根因。交接补齐源码/产物/进程/URL/可见功能五项核验；未停止历史容器、重建服务或修改应用。
- 下一步：按另一窗口URL/实际进程区分旧容器、旧dist或旧端口；强制构建来源保护如需实施另列最小任务。
