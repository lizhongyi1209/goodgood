# GG-023 — Sharp 安全更新

- 状态：Sharp 源码与安全镜像已完成；生产部署延期，交接 PR #6 被新 Next CRITICAL 项阻止
- 来源：累计发布记录合并后的 main 镜像安全扫描
- 最后更新：2026-09-09
- 分支：`fix/GG-023-sharp-security`
- 基线：`572a3b7c6f8ec7f7193a99799921d9387e58c9fd`

## 范围与验收

- 将直接运行时依赖 Sharp 从 0.35.0 升级到修复版 0.35.4，更新锁文件与精确版本契约。
- 保持参考图校验、provider 图片解码、运行时打包和产品行为不变。
- 完整本地门禁与 GitHub CI 通过；Trivy 的 HIGH/CRITICAL 镜像扫描恢复绿色。
- 不改数据库、迁移、价格、账户或生产数据；不发起真实 provider 请求。
- 这是已锁定依赖的安全修复，不改变产品决策，无需新 ADR。

## 实现与证据

- main run `34302821815` 的源码/依赖/构建检查通过；发布镜像的 Trivy 扫描唯一失败项为
  Sharp 0.35.0 的 `GHSA-rgj7-g3m4-5g8c`，严重度 HIGH，扫描器给出的修复版为 0.35.4。
- `npm view sharp@0.35.4 version` 确认固定版本可用；manifest、override 和锁文件现统一
  固定 0.35.4，Sharp 运行时依赖的 libvips 从 1.3.0 更新为 1.3.3。
- 定向 CI/资产/参考图测试 16/16 通过；真实图片解码校验通过，`npm ls` 只解析出
  `sharp@0.35.4`。
- `npm run check:local` 通过：252 项中 246 通过、6 个 opt-in 跳过、0 失败；lint、
  typecheck 和本地构建通过。`git diff --check` 通过。
- PR 首次 CI 在 `npm ci` 阶段发现 Windows npm 11.6 未写入两个跨平台 WASM 的嵌套
  `@emnapi` 锁记录；未进入代码或镜像测试。使用 npm 11.8 重新生成锁文件后，
  `npx -y npm@11.8.0 ci --dry-run --no-audit --no-fund` 通过。
- official npm audit 另列出 Next 16.2.11 的 Windows-hosted RCE 和 Next AVIF optimizer
  advisories：生产运行于 Linux，且 GoodGood 使用 vinext 自有运行时而非 Next 图片优化 API；
  AVIF 的底层 libheif 风险由本次直接 Sharp 更新处理。是否升级 Next 不扩入本次最小修复，
  仍由实际 runtime 镜像 Trivy 门禁 fail-closed 判定。
- PR run `34303864503` 的完整质量门禁与验证镜像扫描通过；PR #4 已合并为 main
  `18fe779b4884561f0d665dea9449528c40348612`。
- main run `34304055572` 的源码门禁和不可变镜像发布均通过；运行时镜像
  `ghcr.io/lizhongyi1209/goodgood@sha256:b441e16685c77842e18cefcdbcae00c2e50d25350fe598ea2e462dd61758f152`
  通过 HIGH/CRITICAL 扫描。artifact evidence ID `10086061523`，摘要
  `sha256:2ebad92671c4f0630dd9d55e03510254ac969b0dc8ec92b27932c9455c129785`。

## 恢复工作

- 尚未完成：新安全镜像尚未部署；生产仍运行 `65ceb168`。
- 2026-09-09 决定：站长明确将生产切换与额外真实冒烟留到下一次发布执行；本轮保持线上
  身份不变，不重新生成发布证据，不新增计费 provider 请求。
- 延期记录 PR #6 的 run `34322442639` 使用更新后的 Trivy 数据库，将 `next 16.2.11` 的
  `CVE-2026-75604` 与 `GHSA-2xp9-vwfh-vxw4` 报为 CRITICAL，修复版为 16.3.3；源码/文档
  步骤此前通过，但文件系统扫描失败，镜像发布跳过。PR 因此保持未合并，生产未受影响。
- 风险：当前线上仍运行 0.35.0。这个风险已明确记录但尚未消除；此前仅一次真实生图授权
  已经使用，不得擅自再次发起计费请求。
- 下一步：下次先核定 Next 16.3.3 升级范围并恢复 fail-closed CI，再取得额外真实冒烟和生产
  发布授权；以届时最新的精确候选重新生成 CI artifact、production preflight/alpha evidence
  并执行兼容的应用层切换，不要自动部署或复用当前归档候选。
