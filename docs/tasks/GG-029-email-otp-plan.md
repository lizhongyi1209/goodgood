...
- 状态：P0 首轮小规模验证闭环：服务、发信域、DNS、发信地址、香港主机 SMTP 认证及两封 QQ/一封 Gmail 真实收件成功；完整跨邮箱/跨时段矩阵留待发布前补齐。P1 与 P2 最小本地候选完成并验证；未部署
...
- 要做：在既定方案上实现隔离本地 P1 闭环和 P2 浏览器/最小日常运维；同步 schema、错误、测试和运维文档。
...
- 授权边界：用户已要求当前窗口开始邮件计划，授权本地实现与验证；并明确提供一个 QQ 测试收件箱、授权只发送一封真实验证码邮件。该次投递已完成；后续真实投递、生产数据迁移和部署仍需按目标与数量明确范围。旧 GG-028 配置授权不扩展为本次迁移上线授权。
...
- P0 凭据边界：认证冒烟所用一次性密码未写入磁盘、终端输出、聊天或 Git；临时浏览器副本与本地桥接进程均已清除。冒烟完成后，用户已在阿里云控制台亲自重新设置 SMTP 密码；新值未被代理读取、留存或挂载。生产秘密挂载尚属 P3，正式部署时只能由用户把当前值直接安装到受保护挂载（若未安全留存则再次轮换），不能把密码重置写成生产凭据已就绪。
...
- 容器构建复验：本轮尝试完整应用镜像时，既有 Docker/npm 链路在执行 P2 代码前因缺少 Linux `@rolldown/binding-linux-x64-gnu` 可选依赖失败；原 Dockerfile 同样使用该安装路径。该问题不影响上述主机运行时/隔离 PostgreSQL 证据，但须在 P3 CI/镜像门禁前单独修复并复验。
- 容器/浏览器：隔离项目 `goodgood-gg029` 构建、迁移、健康检查通过；Mailpit 合成收信端到端返回 pending/100 积分；390×844 Chrome 视口从邮箱输入走到审核页，无真实外发或生图请求。
- 完整门禁：`npm run check:local` 通过，268 项中 261 通过、7 个显式外部/数据库测试跳过、0 失败；同时修正迁移版本断言、Windows CRLF 测试容差和一个既有 O1Key 失败轮询测试的并发超时容差。
- 历史收尾：GG-028 worktree 的任务/ADR/BACKLOG/检查点同步为已被取代，其文档测试也为 8/8；未改动其运行时代码。
- 发布：未发布；未读取/写入线上秘密或业务数据，未改变线上 Authing 登录。

## 恢复工作

- 尚未完成：P0 跨邮箱/跨时段投递表现；P3 受审绑定工具、模式化 production preflight、生产 SMTP 密码轮换与秘密挂载、外部告警送达/CI 镜像；P4 上线与观察。
...
- 下一步：进入 P3 发布准备并先修复/复验 Linux 镜像依赖；随后补模式化 preflight、受审绑定工具、秘密挂载与外部告警送达。发布前再按明确授权补 163/Outlook 与跨时段样本，不密集重复发信。回复能力仍需站长提供真实支持邮箱。生产秘密挂载留在 P3 单独批准范围；不要切生产、清理 Authing 或继续 GG-028 域名修复。
