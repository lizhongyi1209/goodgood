...
- 状态：P0 服务、发信域、DNS、发信地址、香港主机 SMTP 认证冒烟及两封 QQ 真实收件已完成，Gmail 单封测试已获授权待执行，仍待跨邮箱/跨时段验证；P1 本地候选完成、P2 界面/清理部分完成并验证；未部署
...
- P0 第二封真实投递：用户明确授权向另一 QQ 测试收件箱只发送一封验证码邮件；SMTP 接受后用户确认成功收到。项目不记录完整收件地址、验证码、密码或消息 ID；该封收件夹位置和耗时未补充。用户随后明确授权向其 Gmail 测试收件箱只发送一封，尚待本人通过一次性本机输入页提交 SMTP 密码。
...
- 下一步：取得明确授权的 QQ、163、Gmail、Outlook 测试收件箱，按批准的地址和数量继续小规模跨邮箱/跨时段验证；优先增加一个 Gmail 或 Outlook 样本。回复能力仍需站长提供真实支持邮箱。生产秘密挂载留在 P3 单独批准范围；不要切生产、清理 Authing 或继续 GG-028 域名修复。
