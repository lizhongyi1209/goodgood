...
- 状态：P0 首轮小规模验证闭环：服务、发信域、DNS、发信地址、香港主机 SMTP 认证及两封 QQ/一封 Gmail 真实收件成功；完整跨邮箱/跨时段矩阵留待发布前补齐。P1 本地候选完成、P2 界面/清理部分完成并验证；未部署
...
- 尚未完成：P0 跨邮箱/跨时段投递表现；P2 告警/支持汇总和定向账号撤销验收；P3 受审绑定工具、模式化 production preflight/生产 SMTP 密码轮换与秘密挂载/CI；P4 上线与观察。
...
- 下一步：结束本轮真实发信，进入 P2 最小运维补齐；发布前再按明确授权补 163/Outlook 与跨时段样本，不密集重复发信。回复能力仍需站长提供真实支持邮箱。生产秘密挂载留在 P3 单独批准范围；不要切生产、清理 Authing 或继续 GG-028 域名修复。
