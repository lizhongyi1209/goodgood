...
- P0 首封真实投递：用户明确授权向其 QQ 测试收件箱只发送一封验证码邮件；阿里云 SMTP 于 `2026-09-10T14:46:18.923Z` 接受请求，用户随后确认成功收到。记录不包含完整收件地址、验证码、SMTP 密码或供应商消息 ID；收件夹位置和实际延迟尚未记录。一次性本机输入页、发送进程和临时脚本均已关闭或清除。
...
- 下一步：补记首封 QQ 邮件的收件夹位置与大致延迟；之后取得明确授权的 QQ、163、Gmail、Outlook 测试收件箱，按批准的地址和数量继续小规模跨邮箱/跨时段验证。回复能力仍需站长提供真实支持邮箱。生产秘密挂载留在 P3 单独批准范围；不要切生产、清理 Authing 或继续 GG-028 域名修复。
