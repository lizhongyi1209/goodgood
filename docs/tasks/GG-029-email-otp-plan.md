...
- 状态：P1 本地候选完成、P2 界面/清理部分完成并验证；未部署
...
- 不做：本轮不开通真实邮件服务、不改 DNS/生产配置、不读取生产秘密、不删除生产测试数据、不部署；不恢复 Google、完整 C6、支付或复杂监控。
...
- 尚未完成：P0 邮件供应商账号/地域/额度、发信域名和真实收件；P2 告警/支持汇总和定向账号撤销验收；P3 受审绑定工具、模式化 production preflight/秘密挂载/CI；P4 上线与观察。
...
- 下一步：先取得 P0 的实际供应商/DNS/支持邮箱输入并做授权真实收件测试；随后完成 P3 发布准备。不要切生产、清理 Authing 或继续 GG-028 域名修复。
