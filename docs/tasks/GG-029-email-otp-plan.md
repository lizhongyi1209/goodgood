...
- 状态：P0 只读核验进行中；P1 本地候选完成、P2 界面/清理部分完成并验证；未部署
...
- P0 只读核验：`o1key.com` 权威 DNS 为 Cloudflare；`mail.goodgood.o1key.com` 当前没有 TXT、MX 或独立 DMARC 记录，未发现既有邮件配置冲突。阿里云主账号已登录，但 Direct Mail 尚未开通；没有提交开通、购买、域名或凭据操作。
- P0 当前结论：香港主机属于阿里云文档所述的境外部署；若主要面向境内收件箱且不是美国/新加坡部署，首选新加坡区域，并使用匹配区域的 `smtpdm-ap-southeast-1.aliyuncs.com:465`。最终仍须在香港主机做 TLS 连通测试和真实收件验证。
- P0 当前费用边界：官方页面显示主账号总计 2,000 封免费额度、每天最多免费 200 封；超出后按量 2 元/1,000 封。账户实际剩余额度仍须开通后在控制台核对；本任务不购买资源包。
...
- 尚未完成：P0 Direct Mail 按量服务开通、选择新加坡区域、创建发信域名/地址、配置和验证 DNS、受保护 SMTP 凭据、香港主机 TLS 连通及真实收件；P2 告警/支持汇总和定向账号撤销验收；P3 受审绑定工具、模式化 production preflight/秘密挂载/CI；P4 上线与观察。
...
- 下一步：取得按量服务开通授权后只开通 Direct Mail、不买资源包，选择新加坡区域并创建 `mail.goodgood.o1key.com`，再按控制台生成的精确值配置 Cloudflare DNS。另需站长提供真实支持邮箱和明确授权的测试收件箱，之后才能做香港主机 TLS 与真实收件验证。不要切生产、清理 Authing 或继续 GG-028 域名修复。
