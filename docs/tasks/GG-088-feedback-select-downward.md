...
- 验证：check:local通过（lint/typecheck/build，522通过/25显式写测试跳过/0失败）；定向反馈4/4、文档8/8通过。Chrome桌面与390px新表单默认其他首次均bottom，菜单顶部168/164px位于触发按钮底部163.64/159.64px下方；改选生成后再次展开仍bottom。Home/Return选择及Escape返回触发器焦点正常，临时视口已重置、测试标签页已关闭。
