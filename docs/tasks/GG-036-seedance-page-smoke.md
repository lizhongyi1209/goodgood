...
- 用户要求一个企业身份账号查看登录后的样式。通过正常邮箱 OTP API 注册 `enterprise-demo@local.goodgood`，再通过已正常验证登录的站长管理 API 审核为 active、设置 enterprise 业务身份，并建立 `GoodGood 测试企业` 工作区，账号为 org_owner；没有站长角色、没有追加积分或生成任务。
- 工作区 ID：`c17104db-346f-4579-a7e8-aa4117e3facf`。仅操作 `goodgood-gg033-real` 本地测试库，不直接插入 fixture、不改生产数据；验证码与会话凭据只在辅助进程内存使用。
- computer use 在现有 Chrome 退出原站长会话，并打开 `/workspaces/c17104db-346f-4579-a7e8-aa4117e3facf/create`。因 computer-use 技能禁止自动操作用户登录弹窗，停在真实邮箱验证码登录界面交用户完成，不伪造浏览器 Cookie，也不声称已登录企业账号。
- 下一步：用户用上述邮箱在现有页发送验证码，从本地 58045 收件箱读取并登录，检查企业视角。辅助脚本仅位于忽略的 work 目录，不进入发布候选。
