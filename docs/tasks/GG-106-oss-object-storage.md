...
- 状态：**应用已部署生产**（2026-09-24，`d04b727`）；CI、生产预检、单槽切换与公网/私有读核验通过。真实账号浏览器上传、此版本的新生成存储与跨账号拒绝仍待实测；独立 R2 备份保持运行。发布事实见 [GG-106 发布记录](../releases/2026-09-24-gg106-oss-cutover.md)。
...
- 发布后不触发模型调用的只读核验：生产候选代码按历史键访问 R2 对象 HEAD 200，按 `oss/` 键访问 OSS 探针 HEAD 200，使用生产共享密钥生成的 ESA 签名 HEAD 200；ESA 与 OSS 上传域名匿名 HEAD 均 403。浏览器自动化连接失败，真实登录账号的浏览器上传、实际新增 `oss/` 应用记录、新生成结果与跨账号拒绝均**未核对**。本次未授权付费模型冒烟，不将这些项记为通过；`controlled-alpha-operations` 无告警通道的既有 `fail` 仍保持。
