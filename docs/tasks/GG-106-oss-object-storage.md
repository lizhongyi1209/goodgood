...
- 状态：**应用已部署生产**（2026-09-24，`d04b727`）；CI、生产预检、单槽切换与公网/私有读核验通过。已观察到一条新生成素材的 `oss/` 记录并验证私有 HEAD；真实账号浏览器上传、页内预览与跨账号拒绝仍待实测。独立 R2 备份保持运行。发布事实见 [GG-106 发布记录](../releases/2026-09-24-gg106-oss-cutover.md)。
...
- 独立候选工作树 `F:\goodgood-gg106-release` 建于 GG-100 单槽基线 `de699e1`，仅移植 GG-106 的应用、配置、ESA 和验证文件；GG-104 才有的视频素材模块未带入。OSS/预检定向测试 13/13、文档连续性 8/8、`npm run check:local` 542 通过/26 隔离跳过/0 失败。此时尚未安装生产密钥、构建发布镜像或部署；不得把原开发分支累积的 GG-101—105 应用代码一并发布。
- 独立候选分支已推送至 `origin/codex/GG-106-scoped-release`。生产主机只读 SSH 与 `sudo` 可用；三个新密钥文件已从本机受控来源经 SSH 输入安装至 `/etc/goodgood/production/secrets/`，均为 root:986、0640 且非空，未输出密钥值。现有 R2 凭据文件保留。受保护的运行配置尚未切换，运行容器未重启；候选 CI 不可变镜像和预检仍待完成。
- 发布边界补齐：GG-098 不认识新 `oss/` 键，切换后不能直接回退到 GG-098。ADR 0098 确认故障时同一双读版本可显式启用 `OBJECT_STORAGE_EMERGENCY_R2_WRITES=true`，新键临时写回 R2，旧 R2 与已有 OSS 键仍按前缀读取；正常生产预检只接受 `false`。已增加上传签名、对象路由、Bucket 验证与预检定向覆盖；代码更改后 OSS/预检 13/13、文档连续性 8/8、完整 `npm run check:local` 542 通过/26 隔离跳过/0 失败。生产运行配置仍未改动。
- 2026-09-24 生产发布：站长明确同意合入独立候选，并在 CI 不可变镜像、生产预检及单槽检查通过后发布。`main` 快进到 `d04b72752065d3ecc31b7b5bc7ca7ecc0988ebfa`；CI run `35983011527` 两个 job 成功，镜像 digest `sha256:d183628b…03a6e`，工件 `10801121979` 完整性校验通过。主机三个新密钥文件与旧 R2 凭据分离；受保护配置与 Compose 已备份后更新，`OBJECT_STORAGE_EMERGENCY_R2_WRITES=false`。生产预检所有检查通过，SMTP 仅鉴权不发信。
- 切换窗口：活动任务、未完成上传、冻结账户均为 0；新备份快照 `3b0a7627`，Restic `check` 与无网络/tmpfs 隔离恢复演练通过（59 表、6817 行、44 迁移）。维护页 503 后停止旧 Worker/Web，迁移保持 44，启动新 Web/唯一 Worker；双方 readiness 200、revision 一致、重启 0。维护标记移除后公网 `/`、`/login`、`/register` 200，未登录会话 401；timer 仍 `enabled`/`active`。
- 发布后不触发模型调用的只读核验：生产候选代码按历史键访问 R2 对象 HEAD 200，按 `oss/` 键访问 OSS 探针 HEAD 200，使用生产共享密钥生成的 ESA 签名 HEAD 200；ESA 与 OSS 上传域名匿名 HEAD 均 403。浏览器自动化连接失败，真实登录账号的浏览器上传/预览与跨账号拒绝均**未核对**。本次未授权付费模型冒烟，不将这些项记为通过；`controlled-alpha-operations` 无告警通道的既有 `fail` 仍保持。
- 随后出现 1 条新生成素材的 `oss/` 数据库记录、任务状态 `succeeded`；只读 HEAD 核对该实际对象：OSS 200、应用签发 ESA 200、匿名 ESA 403。未读取图片内容、未由发布探测触发模型调用。浏览器直传/预览与跨账号拒绝仍未核对。
