...
| 阶段 | 状态 | 交付物 |
| --- | --- | --- |
| 0 | 实施中 | 隔离工作树、合并预检、任务契约和本地边界 |
| 1 | 待开始 | 合入 GG-029/GG-030、解决冲突、恢复构建与定向测试 |
| 2 | 待开始 | 统一邮箱身份语义、数据库/API 集成测试 |
...
提交阶段 0 契约，然后依次合入 GG-029 与 GG-030；冲突只在 GG-031 解决，不回写来源分支。
