...
GG-096以ADR0088取代GG091的单一认证表单界面：本地候选提供独立`/login`、`/register`与固定登录/注册子导航；登录隐藏邀请码，注册直接填写邀请码。GG091账户规则不变：每账户固定唯一6位数字邀请码、可无限邀请，显示本人积分下方；新用户与旧pending必须活动账户的邀请码，老active只需邮箱验证。邀请码为分享标识，不授予角色或奖励；尚未部署。线上测试账户清理已独立执行，见GG091任务。
