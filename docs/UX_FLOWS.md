...
GG-091统一表单：邮箱→发送验证码→填写邮件码/新用户邀请码→确认。邮箱可以直接改，新地址须重新发码；异步发信响应仅绑定请求时地址。缺码/无效码不注册，正确邮件码可修正邀请码重试；既有active无需填写邀请码。每账户固定六位码可无限邀请，本人菜单积分下方复制；暂停邀请者的码不可用于新注册。后台不再额外发码。
...
GG-086发行进度按本期累计已发放/限定额度，退款回收不倒退；站长计划15秒只读轮询，隐藏页面停止，恢复可见立即刷新。操作/确认期间暂停；轮询不隐藏卡片，失败保留快照并提示，可重试/自动恢复；更新时间含北京时间秒数。

GG-084个人平台币页从桌面左下或手机账户菜单进入，保持当前创作；自己的余额/累计获得/累计撤回和获得记录，不展示平台额度或倒计时。未获币显示零与安静说明；加载/失败/刷新/加载更多可恢复，登录失效回到登录状态。记录显示原消费/退款时间和正负币数，充值消费记录附有效积分。站长管理平台币页按实际批次卡片展示计划（GG-085当前仅真实第一期），期号/状态、额度/剩余、规则与原操作归属同一张卡片；全平台库存及未安排额度在卡片外。开启需Radix确认对话框；失败保留同一操作键，成功刷新计划/审计，暂停与恢复沿用原批次。尚未到9.18时显示等待起算；发完不给重开入口，下一批另行实施。
