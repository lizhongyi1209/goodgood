...
GG-084个人平台币页从桌面左下或手机账户菜单进入，保持当前创作；自己的余额/累计获得/累计撤回和获得记录，不展示平台额度或倒计时。未获币显示零与安静说明；加载/失败/刷新/加载更多可恢复，登录失效回到登录状态。记录显示原消费/退款时间和正负币数，充值消费记录附有效积分。站长管理平台币页显示一期计划，开启需Radix确认对话框；失败保留同一操作键，成功刷新计划/审计，暂停与恢复沿用原批次。尚未到9.18时显示等待起算；发完不给重开入口，下一批另行实施。
