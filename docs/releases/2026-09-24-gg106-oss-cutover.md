...
- 浏览器自动化连接失败，尚未核验真实账号的浏览器直传、应用生成的新 `oss/` 数据库记录、页内预览及跨账号拒绝。用户资产不作为自动探针。模型生成可能计费，本次没有单独授权，故未调用模型。这些是明确的验收余项，不算已通过。
...
下一步最小验收：由已登录的正式账号上传一张非私人测试图，核对 `oss/` 记录、OSS 私有对象、本人预览与未登录拒绝；另用不同账号核对所有者隔离。真实生成存储需另获计费测试授权。
