...
- 当前工作：GG-097累计功能生产重发布**已完成并开放**。累计功能（GG-024—GG-096）已从 `65ceb168`/`0019` 前移到 `5b65601`/`0043`。**注册开关已于 2026-09-15 由站长要求打开（`GOODGOOD_EMAIL_REGISTRATION_ENABLED=true`）**，即任何人经邮箱验证码注册后立得 200 欢迎积分并可立即创作，准入收口仅剩该开关（见 ADR 0090）。站长账户 951565127@qq.com，邀请码 405513。
...
- 当前无生产站长测试账户；本地邀请码/新认证入口尚未部署。唯一工作区测试服务为32131，GG096完成切换后使用`http://127.0.0.1:32131/login`与`/register`，邮箱验证码且不预设账户登录状态；邮件查看入口为 `http://127.0.0.1:58045`。不自动注册或重置数据。完整记录：[GG091清理](operations/2026-09-14-gg091-test-user-cleanup.md)。
...
- 本地最新代码是GG092交接检查点，main不包含GG081—091累计功能；新功能从该检查点接续。main或分支名不等于生产版本，
