...
- 当前工作：GG-094在分支fix/GG-094-registration-invitation-visibility修正本地邮箱登录界面：登录状态隐藏邀请码，只有待注册/开通时显示；当前等待用户在32191手动验收。累计功能仍未部署，GG023安全镜像仍未切生产。
- 下一个普通产品需求从 GG-095 分配，以BACKLOG核验占用；不要自动恢复搁置的 C6。
...
- 当前无站长测试账户；本地邀请码/新登录界面尚未部署。GG094登录 UI 检查入口为 `http://127.0.0.1:32191/create`，邮件查看入口为 `http://127.0.0.1:58045`；不自动注册或重置数据。完整记录：[GG091清理](operations/2026-09-14-gg091-test-user-cleanup.md)。
