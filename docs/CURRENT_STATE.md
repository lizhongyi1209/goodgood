...
- 当前工作：GG-094登录/注册邀请码界面已获用户验收；GG-095在分支chore/GG-095-primary-email-auth-test-flow将32131调整为无预设账户的标准邮箱验证码测试入口，并停用独立32191。累计功能仍未部署，GG023安全镜像仍未切生产。
- 下一个普通产品需求从 GG-096 分配，以BACKLOG核验占用；不要自动恢复搁置的 C6。
...
- 当前无站长测试账户；本地邀请码/新登录界面尚未部署。GG095完成后唯一工作区测试入口为 `http://127.0.0.1:32131/create`，使用邮箱验证码且不预设账户登录状态；邮件查看入口为 `http://127.0.0.1:58045`。不自动注册或重置数据。完整记录：[GG091清理](operations/2026-09-14-gg091-test-user-cleanup.md)。
