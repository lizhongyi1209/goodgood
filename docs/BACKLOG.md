...
最后同步：2026-09-13。这里是索引，细节以任务卡为准；不是自动执行所有项目的授权。

## 当前与最近交付

| ID | 事项 | 状态 | 入口 |
| --- | --- | --- | --- |
| GG-055 | Gemini 官方单张图片成本核对 | 本地分析及文档验证完成（15/15）；区分图片输出/整单成本、人民币与积分测算，未改价/发布 | [任务](tasks/GG-055-gemini-image-costs.md) |
| GG-054 | Banana 三线路与分线路定价 | Pro 与通用选线/定价本地完成；门禁 423 通过/16 opt-in 跳过、SQL/界面验证通过，32141 保留；Banana 2 两条 ID 待补齐，未发布 | [任务](tasks/GG-054-banana-lines.md) |
...
GG-024—GG-055 已由任务占用（GG-047 为另一工作树的 agent 范围配置）；下一个普通产品需求从 **GG-056** 分配并检查是否已被占用。每次 alpha 发布仍须取得
