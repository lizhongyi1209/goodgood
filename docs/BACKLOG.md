...
| ID | 事项 | 状态 | 入口 |
| --- | --- | --- | --- |
| GG-050 | 移除页面右上角常驻返回入口 | 本地完成；门禁 403 通过/14 opt-in 跳过、原 Chrome 企业/积分/资产检查通过，待用户验收；不新增替代导航，未发布 | [任务](tasks/GG-050-remove-page-return-actions.md) |
...
GG-024—GG-050 已由隔离任务占用（GG-047 为另一工作树的 agent 范围配置）；下一个普通产品需求从 **GG-051** 分配并检查是否已被占用。每次 alpha 发布仍须取得
