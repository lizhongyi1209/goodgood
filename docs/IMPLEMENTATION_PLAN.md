...
- P0 已获用户授权并开通 Direct Mail 按量服务，没有购买资源包；账号正常，日/月额度为
  ...
  密码尚未保留或挂载。首封经明确授权的 QQ 验证码邮件已获 SMTP 接受且由用户确认收到；完整
  地址、验证码、密码与消息 ID 均未写入项目，收件夹位置与大致延迟待补记。
...
- 支付、自动账户删除、举报、完整外部删除条款与完整 seed readiness 仍在 GG-900—GG-902
  搁置范围，本次发布没有恢复它们。
- Next action: 补记首封 QQ 邮件的收件夹位置与大致延迟；取得明确授权的 QQ、163、Gmail、Outlook
  测试收件箱后继续小规模跨邮箱/跨时段 P0 验证。P3 再单独批准生产 SMTP 秘密挂载，回复能力仍需真实支持邮箱。
