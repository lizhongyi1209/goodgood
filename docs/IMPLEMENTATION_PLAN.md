...
- Current objective: GG-027 在 GG-026 本地整合候选上分四个代码阶段实现企业/分销直属下级积分划拨；
  阶段 0 决策与契约已确认，先做充值来源守恒，再做身份关系、零和转移和页面联调。所有工作留在本地，
  当前线上 `65ceb168` 保持不变。
...
- Next action: 实现 GG-027 阶段 1 的积分来源投影与生成 reserve/settle/release/refund 来源守恒迁移，
  用定向账务和迁移测试证明既有欢迎/测试积分不会成为可分配积分。
...
| GG-027 | 阶段 0 已完成 | 分销边界与四阶段本地计划已固化；阶段 1 待实现 |
