...
- 3030 已先备份再把原数据卷从 0019 向前迁移到 0022，并运行 `goodgood:gg027-local@39b9cc3`。本地线下付款订单 `ord_e817175300f34e02b06a698ea9c65524` 提供 500 个充值来源积分；50 积分划拨 `trf_5ad6a603bc70402b8adafd71ab6d4e16` 的余额、来源、双边账本、幂等重放、普通账户 403、积分记录和消费汇总均验证通过。桌面与 390×844 浏览器复验通过；窄屏页头溢出在 `455fb29` 修复并经 7/7 定向回归确认。三个账户管理下拉列表已固定向下展开；状态统计卡已移除、首屏默认全部账户，身份显示收敛为 `站长 / 个人 / 企业 / 分销商` 单一标签，站长行不再提供身份/上级操作。宽屏表格现将账户、身份和上级拆为独立列，中窄屏使用独立字段。首页左下账户卡现按 A 方案用首行用户名、次行静态身份标签加宫墙红积分显示当前身份与余额；完整本地门禁 284 项中 275 通过、9 个 opt-in 跳过、0 失败，3030 readiness 与桌面浏览器复验通过，验收过程未提交账户变更。
