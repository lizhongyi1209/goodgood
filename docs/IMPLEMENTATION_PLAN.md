...
- Current phase: GG-051 人民币锚定 Runway 计价方向获认可，积分单位转换与售价评估完成。
- Current objective: 评估 1 元/100 新积分的等值方案与按张/输出秒/参考秒销售规则；具体单位/费率待定，不实施价格或数据变化。

## Current checkpoint

- 当前工作树：`chore/GG-051-credit-pricing-reassessment`（`F:/goodgood-worktrees/GG-051`），按 GG-051 接续卡从 GG-050 `903552e` 隔离建立，已验证 main `bab17fd` 为祖先；旧 GG-037 工作树与 `.codex/` 配置保留。用户认可的人民币按规格计价方向已记录 ADR 0062，单位与费率尚未执行。
...
- GG-050 已删除五类头部入口及无用接线/样式；定向加文档 43/43，稳定后一次完整门禁 417 项中 403 通过/14 opt-in 跳过/0 失败，diff 检查通过。仅重启原 32140 Web（会话 `6165`），readiness 五项 ok、视频 available true/persistence false；原 Chrome 企业团队资产、积分记录、资产库头部确认和最终团队资产截图完成，页面保留。真实站长/分销会话与窄屏未实测；数据/权限/Worker 保持，不新增替代导航。
- 2026-09-13 GG-051 第四轮：用户接受 Runway 人民币按规格价格策略，ADR 0062 已记录；1 元/100 积分推荐以旧积分乘 2 等值转换，模型人民币售价与单位转换分别评估。视频按输出秒加参考输入秒，平台承担固定报价成本差异；面板人民币编辑/积分换算仍为方案。详见[研究记录](research/GG-051-credit-pricing-reassessment.md)，无价格/余额/服务变化。
- Next action: 确定 1 元/100 新积分与等值转换，补齐各线路完整参数成本，制定人民币规格价目表后明确站长面板与正式视频结算实施范围。
- Blockers: 兑换比例和具体费率尚未正式确定，完整规格成本/波动与风险承担缺证据；视频正式计价与持久链路尚未实施。

## Verification sequence

1. GG-050 定向 43/43、完整门禁 403 通过/14 opt-in 跳过；交付文档复验 8/8、diff 检查通过，不启用数据库写 opt-in。
2. GG-051 为研究文档，执行文档契约与 diff 检查；不重跑代码门禁、不重启服务、不操作浏览器/provider。精确结果见任务卡。
...
| GG-050 | 本地完成并获用户验收 | 删除五类页面常驻返回入口，保留关闭/错误恢复/新建创作，不新增导航，未发布 |
| GG-051 | 人民币 Runway 方向获认可 | ADR 0062 与等值单位/售价分析完成，具体兑换和费率待定；未实施/改价/发布 |
...
2. GG-051 研究接续打开 `F:/goodgood-worktrees/GG-051`，读任务卡与研究记录；GG-037 保留原候选/服务，`F:/goodgood` 仍是旧 GG-024，不误恢复旧 C6。当前展示页为 32140 企业团队资产，不填充真实数据。
