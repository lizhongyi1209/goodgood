...
- GG-013：活动骨架和完成图改用同一创作网格，稳定槽位在提交时形成并原位替换；
  创作卡片显示具体像素，创作卡片与图片详情移除重复收藏和所有按序号调色；创作卡片下载按钮补齐交互反馈，本地签名对象 CORS 补齐 `GET/HEAD` 并拒绝空 Blob，再以系统保存选择器/Blob 回退下载。完整门禁
  226 项（220 通过、6 项 opt-in 跳过），3010 Web 健康且浏览器静态结果复核通过；
