...
- Current objective: GG-004—GG-008 真实 O1Key 本地组合候选重建完成后，供站长在 3010 实测验收。
...
- Next action: 完成 3010 重建；站长实测 GG-004—GG-008。
  若批准发布则先完成 GG-003 门禁，再审查正式发布候选。
...
| GG-008 | 待实测（组合门禁已通过） | 快速重复提交、独立任务状态与逐任务失败恢复 |
