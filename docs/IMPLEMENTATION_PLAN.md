...
- Current objective: GG-027 在 GG-026 本地整合候选上分阶段实现企业/分销直属下级积分划拨；
  阶段 0—4 代码和唯一一次完整本地门禁已完成，下一步执行 3030 保留数据迁移与浏览器联调。所有工作留在本地，
  当前线上 `65ceb168` 保持不变。
...
- Next action: 备份并迁移 3030 保留数据，准备本地充值来源演示积分并完成桌面/窄屏浏览器验收；不得发布线上。
...
| GG-027 | 阶段 0—4 与完整门禁已完成 | 分销边界、来源守恒、身份关系、原子划拨与页面已验证；待 3030 联调 |
