...
- Current objective: GG-026 正在把 GG-024 账户管理视觉修复与 GG-025 积分记录合入同一隔离分支，
  保留 3030 的积分和站长测试数据，并在联合验证后停止 3020 环境；当前线上 `65ceb168` 保持不变。
...
- 支付、自动账户删除、举报、完整外部删除条款与完整 seed readiness 仍在 GG-900—GG-902
  搁置范围，本次发布没有恢复它们。
- GG-026 从同一 `origin/main@42fc8d81` 基线建立集成分支；当前已合入两组功能代码，下一步运行联合
  定向测试和完整门禁，再以唯一镜像标签重建 3030 并停止 3020 容器（保留其数据卷）。
- Next action: 完成 GG-026 联合验证与单环境浏览器验收；后续推送或生产发布仍需要新授权。
...
| GG-026 | 实施中 | 合并 GG-024/GG-025 并把本地预览整合为 3030 单环境 |
