...
- 为阶段 4 手工实测，专用 Compose 项目已在新数据卷上重新启动并保留：GoodGood
  `http://127.0.0.1:32131`、Mailpit `http://127.0.0.1:58031`、PostgreSQL `127.0.0.1:55231`；
  详细的逐步操作、预期结果与停止条件见 [GG-031 任务卡](tasks/GG-031-email-enterprise-integration.md)。
...
| GG-031 | 阶段 4 手工进行中 | 自动化本地集成已通过，正在按任务卡逐步进行老板/员工手工实测；线上未开始 |
