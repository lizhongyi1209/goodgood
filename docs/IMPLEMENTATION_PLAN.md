...
- Current phase: GG-046 模拟门禁通过，但用户认为不直观；已切回真实本地企业账户概览供用户查看。
- Current objective: 沿用真实企业会话和完整管理布局展示现有数据，不注入模拟身份或改动账本。
...
- Next action: 用户在原 32140 企业概览检查完整管理页面并反馈；5173 模拟实现保留但不是当前展示入口。新增真实测试数据需明确范围；视频定价/持久化/素材接口仍未接入。
- Blockers: 无；素材与正式持久/计价链路按用户要求后续处理。

## Verification sequence

1. GG-046 定向 27/27、完整门禁 396 项中 382 通过/14 跳过；不启用 opt-in 写测试或提交 provider 任务。
2. 原 Chrome 单标签已从 5173 模拟页切回 32140，确认真实企业会话与完整概览出现；仅导航/读取，没有 mutation。32138 历史结果页保持。本次仅文档更新，不重复源码完整门禁。
...
2. 从 `feature/GG-046-business-style-preview` 恢复；先读 GG-046 任务卡与 ADR 0058，不恢复旧 C6。当前用户展示入口为 32140 真实企业概览；5173 模拟版本未获认可，不向真实接口填充模拟数据。
