...
- Current objective: GG-025 已在隔离分支完成积分记录页、只读账本投影、完整本地门禁与模拟链路验收；
  当前等待用户验收，不自动合并、推送或改变线上 `65ceb168`。
...
- 支付、自动账户删除、举报、完整外部删除条款与完整 seed readiness 仍在 GG-900—GG-902
  搁置范围，本次发布没有恢复它们。
- Next action: 用户验收 GG-025 的本地预览；后续合并、推送或发布需要新授权。
...
| GG-025 | 本地已验证 | 用户积分记录、只读账本投影与本地模拟链路；待用户验收 |
