...
- Current objective: 延续GG083且完整继承a73835f，记录第一期100万枚额度已接受，按建议系数10测算10万元合格消费；倍率/执行规则待定。
...
- [ADR0082](decisions/0082-jcoin-first-batch-budget.md) Accepted：第一期100万枚（总上限1%/回馈池2%）。[GG083思路](research/GG-083-jcoin-batch-accumulation.md)建议同批固定比例，系数10及最后一笔/批间消费/退款规则未接受。20%创作分母/含义与企业归属待明确，保持未启动。
- 用户“同意。开始讨论”后展开固定100万枚下5/10/20的三档比较，需2000万/1000万/500万有效消费积分发完，优先建议10；参数内存试算通过，没有真实消费统计或发完日期预测。
- 最新问消费金额：按当前100积分/CNY和建议系数10，1000万有效消费积分即10万元合格消费，正常无退款反算通过；这是服务消费规模，不是币的人民币估值。
...
- Next action: 首期100万枚已定；讨论并接受固定比例及批次边界/退款建议，再实施充值消费挖矿、独立账本和累计界面，验证9.18/幂等/库存。下一批另行开启，不接兑换/面值/自动跨批。当前不部署或改历史来源。
- Blockers: 整理思路无阻碍；倍率/执行规则未接受，当前无发行运行时，未授权真实发币或部署。
...
| GG-083 | 结构/首批额度接受，倍率待讨论 | 第一批100万枚，无期限/发完另开下一批/仅累计；倍率与执行规则未接受，未实施 |
