...
- Current objective: GG-027 在 GG-026 本地整合候选上分阶段实现企业/分销直属下级积分划拨；
  阶段 0—3 已完成决策契约、充值来源守恒、业务身份/直属关系和原子零和划拨，下一步做页面与本地联调。所有工作留在本地，
  当前线上 `65ceb168` 保持不变。
...
- GG-027 阶段 2 已新增迁移 0021、站长身份/直属关系 API、唯一有效任期约束、循环检查和审计读模型；
  快速测试与独立 PostgreSQL 全迁移用例通过，临时数据库已删除，3030 和生产仍未迁移。
...
- Next action: 实现 GG-027 阶段 4 的站长操作与企业/分销工作区，完成响应式/错误状态后只运行一次完整本地门禁，并更新 3030 隔离预览；不得发布线上。
...
| GG-027 | 阶段 0—3 已完成 | 分销边界、来源守恒、身份关系与原子划拨已验证；阶段 4 待实现 |
