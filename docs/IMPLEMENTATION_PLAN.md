...
- P0 已获用户授权并开通 Direct Mail 按量服务，没有购买资源包；账号正常，日/月额度为
  ...
  发布前补齐。
- 登录页面按设计系统完成首轮视觉收敛：使用 Double G/自定义字标、柔和浅色画布、紧凑白卡片、
  48px 邮箱/按钮/验证码控件、中文友好字体和克制 Palace Red 状态。邮箱与验证码步骤已在桌面和
  真实 390×844 Chrome 视口检查，六格验证码完整且文案换行正常；未改变登录行为或既定产品决策。
...
| GG-029 | P0 真信与真实应用登录闭环、P1/P2/P3 本地代码已验证 | 自建邮箱 OTP；视觉已收敛，发布前外部证据、生产演练及 P4 待完成 |
