...
- P0 已获用户授权并开通 Direct Mail 按量服务，没有购买资源包；账号正常，日/月额度为
  ...
  状态“正常”的触发邮件地址 `no-reply@mail.goodgood.o1key.com`，可选回信地址留空；SMTP 凭据
  和真实投递尚未提交。
- 香港生产主机到 `smtpdm-ap-southeast-1.aliyuncs.com:465` 的只读连接已成功协商 TLS 1.3，
  证书校验为 OK；未使用 SMTP 密码或发送邮件。
...
- 支付、自动账户删除、举报、完整外部删除条款与完整 seed readiness 仍在 GG-900—GG-902
  搁置范围，本次发布没有恢复它们。
- Next action: 在动作时确认后创建持久 SMTP 凭据并按生产秘密处理，再验证 SMTP 认证；真实投递
  仍需授权测试收件箱，回复能力仍需站长提供真实支持邮箱。
- Blockers: P1 本地实现无阻塞；P0 的 DNS 与发信地址已闭环，当前尚缺动作时确认的 SMTP 凭据、
