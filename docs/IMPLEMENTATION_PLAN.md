...
- Current phase: GG-051 第三轮官方具体价目表核对完成，混合计费/站长面板待讨论。
- Current objective: 按用户要求提供实际平台扣费参照；据此继续讨论 GoodGood 规则，不实施或发布价格变化。

## Current checkpoint

- 当前工作树：`chore/GG-051-credit-pricing-reassessment`（`F:/goodgood-worktrees/GG-051`），按 GG-051 接续卡从 GG-050 `903552e` 隔离建立，已验证 main `bab17fd` 为祖先；旧 GG-037 工作树与 `.codex/` 配置保留。研究方案见 GG-051，现有 ADR 不变。
...
- 2026-09-13 GG-051 第三轮：按用户要求核对 Runway/Adobe/Magnific 具体图片/视频积分表，Runway Seedance 2.5 明确按输入视频秒数附加；公开创作规则不等于官方 API token 成本。第二轮混合计费与面板为 GoodGood 待讨论建议，未宣称是主流统一规则。详见[研究记录](research/GG-051-credit-pricing-reassessment.md)，无价格/余额/服务变化。
- Next action: 确认混合计费、站长面板与预付上限风险承担；补齐 O1Key 成本/usage/上界证据，先以 ADR 定义具体决定，再实施固定与按量各一线路的发布到结算闭环。
- Blockers: 具体单价、用量证据和上游硬上界尚未核实；客户上限超额风险政策未确定，视频正式计价与持久链路尚未实施。
...
| GG-051 | 第三轮具体价目表核对完成 | 官方扣费参照已整理，混合计费/站长面板待讨论；未实施/改价/发布 |
