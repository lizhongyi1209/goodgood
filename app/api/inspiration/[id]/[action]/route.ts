...
export async function POST(request:Request,{params}:{params:Promise<{id:string;action:string}>}) {
  ...
  if(!['like','use','withdraw'].includes(route.action)) {const failure=inspirationApiError(new InspirationError('INSPIRATION_NOT_FOUND','案例入口不存在。',404));return Response.json(failure.body,{status:404,headers:{'cache-control':'no-store'}});}
