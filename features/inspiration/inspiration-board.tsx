...
import {inspirationRequest,type InspirationCase,type CaseImage,type CaseParameters,type UseCaseResult} from './http-inspiration-boundary';
import './inspiration.css';

export function CaseComparison({before,after,title}:{before:CaseImage|null;after:CaseImage;title:string}) {
  return <div className={`case-comparison ${before?'has-before':''}`}>
    {before&&<figure><PrivateObjectImage src={before.url} alt={`${title} · 处理前`}/><figcaption>处理前</figcaption></figure>}
    <figure><PrivateObjectImage src={after.url} alt={`${title} · 处理后`}/><figcaption>{before?'处理后':'效果图'}</figcaption></figure>
  </div>;
}
