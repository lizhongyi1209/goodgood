...
import {Eye,LoaderCircle,Search,Sparkles,Trash2,X} from 'lucide-react';
...
export function InspirationCards({items,onOpen}:{items:readonly InspirationCase[];onOpen:(id:string)=>void}) {
...
export function InspirationBoard({enabled,onUse}:{enabled:boolean;onUse:(value:UseCaseResult)=>void}) {
  ...
  const viewInteraction=useRef('');const openTrigger=useRef<HTMLButtonElement|null>(null);const moreRequest=useRef(0);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    void inspirationRequest<InspirationCase>(`/${selectedId}/view`,{interactionId:viewInteraction.current}).then(value=>{if(active) {setDetail(value);setDirectory(old=>old?{...old,items:old.items.map(item=>item.id===value.id?{...item,views:value.views,uses:value.uses}:item)}:old);setDetailFailure(null);}}).catch(failure=>{if(active) setDetailFailure({id:selectedId,message:failure instanceof Error?failure.message:'案例暂时不可用。'});});
    return ()=>{active=false;};
  ...
  function open(id:string) {viewInteraction.current=crypto.randomUUID();openTrigger.current=document.activeElement instanceof HTMLButtonElement?document.activeElement:null;setDetail(null);setDetailFailure(null);setActionError(null);setSelectedId(id);}
  async function more() {
  ...
  async function use() {
    ...
    try {const value=await inspirationRequest<UseCaseResult>(`/${current.id}/use`,{interactionId:crypto.randomUUID()});setSelectedId(null);onUse(value);} catch(failure) {setActionError(failure instanceof Error?failure.message:'案例暂时不可用。');} finally {setUseBusy(false);}
  }
  ...
  return <section className="inspiration-board" aria-label="灵感板"><header className="inspiration-heading"><div><h1>灵感板</h1><p>把喜欢的效果，变成自己的创作。</p></div><form onSubmit={event=>{event.preventDefault();setDirectory(null);setError(null);setQuery(search.trim());setRevision(value=>value+1);}}><Search size={16}/><input aria-label="搜索案例" placeholder="搜索案例" value={search} maxLength={100} onChange={event=>setSearch(event.target.value)}/><button type="submit">搜索</button></form></header>
    ...
    <Sheet open={Boolean(selectedId)} onOpenChange={value=>{if(!value&&!useBusy) setSelectedId(null);}}><SheetContent className="case-detail-sheet" showCloseButton={false} onCloseAutoFocus={event=>{event.preventDefault();openTrigger.current?.focus();}}><SheetHeader><SheetTitle>{current?.title??'案例详情'}</SheetTitle><SheetDescription>查看效果与原作设置，载入后替换自己的参考图。</SheetDescription></SheetHeader><button className="case-sheet-close" aria-label="关闭案例详情" disabled={useBusy} onClick={()=>setSelectedId(null)}><X size={18}/></button>
      {!current?<InspirationReadState loading={detailFailure?.id!==selectedId} error={detailFailure?.id===selectedId?detailFailure.message:null} onRetry={()=>{setDetailFailure(null);setDetailRevision(value=>value+1);}}/>:<div className="case-detail-body"><CaseComparison before={current.before} after={current.after} title={current.title} mode={current.comparisonMode}/><div className="case-author"><ProfileAvatar className="account-profile-avatar" name={current.author.displayName} url={current.author.avatarUrl}/><div><strong>{current.author.displayName}</strong>{current.author.handle&&<span>@{current.author.handle}</span>}</div></div>{current.description&&<p className="case-description">{current.description}</p>}<section><h3>提示词</h3>{current.promptVisibility==='hidden'?<p className="case-preset-note"><strong>预设 · 提示词隐藏</strong>使用时可补充要求，也可留空。</p>:<p className="case-prompt">{current.prompt}</p>}</section><section><h3>原作参数</h3>{current.parameters?<CaseParametersList parameters={current.parameters}/>:<p className="case-preset-note">参数和提示词已隐藏，使用时保留固定预设。</p>}</section>
        {actionError&&<div className="inspiration-action-error" role="alert">{actionError}</div>}<footer className="case-detail-footer">{current.canWithdraw&&<button className="case-withdraw" disabled={busy.includes(current.id)} onClick={()=>setRemove(current)}><Trash2 size={15}/>{current.owned?'撤回分享':'下架案例'}</button>}<button className="case-use" disabled={useBusy} onClick={()=>void use()}>{useBusy?<LoaderCircle size={16}/>:<Sparkles size={16}/>}使用此效果</button><p>{current.parameterVisibility==='hidden'?'使用固定预设；仅可补充要求和替换参考图。':current.promptVisibility==='hidden'?'使用预设和参数；可补充要求，确认积分后再生成。':'载入提示词和参数；请替换参考图，确认积分后再生成。'}</p></footer></div>}
    </SheetContent></Sheet>
