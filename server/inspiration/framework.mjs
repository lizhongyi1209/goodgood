...
export async function handleInspirationFrameworkRequest(request,route) {
  ...
  try {
    ...
    if(request.method==='POST') {
      ...
      if(new TextEncoder().encode(body).length>8192) throw new InspirationError('INSPIRATION_INVALID','案例请求内容过大。');
  ...
  } catch(error) {const failure=inspirationApiError(error);return Response.json(failure.body,{status:failure.status,headers});}
}
