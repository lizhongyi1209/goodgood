...
export const jcoinBatches=pgTable('jcoin_batches',{
  id:uuid('id').primaryKey(),number:integer('number').notNull().unique(),budgetAtoms:bigint('budget_atoms',{mode:'bigint'}).notNull(),rewardAtomsPerCredit:bigint('reward_atoms_per_credit',{mode:'bigint'}).notNull(),startsAt:timestamp('starts_at',{withTimezone:true}).notNull(),status:text('status').notNull().default('draft'),issuedAtoms:bigint('issued_atoms',{mode:'bigint'}).notNull().default(BigInt(0)),recoveredAtoms:bigint('recovered_atoms',{mode:'bigint'}).notNull().default(BigInt(0)),activatedAt:timestamp('activated_at',{withTimezone:true}),exhaustedAt:timestamp('exhausted_at',{withTimezone:true}),updatedAt:timestamp('updated_at',{withTimezone:true}).notNull().defaultNow(),
},t=>[check('jcoin_batches_status_check',sql`${t.status} in ('draft','active','paused','exhausted')`),check('jcoin_batches_amount_bounds',sql`${t.number}>0 and ${t.budgetAtoms}>0 and ${t.rewardAtomsPerCredit}>0 and ${t.issuedAtoms} between 0 and ${t.budgetAtoms} and ${t.recoveredAtoms} between 0 and ${t.issuedAtoms}`),check('jcoin_batches_first_rules',sql`${t.number}<>1 or (${t.budgetAtoms}=100000000000000 and ${t.rewardAtomsPerCredit}=2000000 and ${t.startsAt}='2026-09-17T16:00:00Z')`),check('jcoin_batches_activation',sql`(${t.status}='draft' and ${t.activatedAt} is null) or (${t.status}<>'draft' and ${t.activatedAt} is not null)`),check('jcoin_batches_exhaustion',sql`(${t.status}='exhausted' and ${t.issuedAtoms}=${t.budgetAtoms} and ${t.exhaustedAt} is not null) or (${t.status}<>'exhausted' and ${t.exhaustedAt} is null)`)]);
export const jcoinAccounts=pgTable('jcoin_accounts',{
  ownerId:uuid('owner_id').primaryKey().references(()=>users.id),balanceAtoms:bigint('balance_atoms',{mode:'bigint'}).notNull().default(BigInt(0)),earnedAtoms:bigint('earned_atoms',{mode:'bigint'}).notNull().default(BigInt(0)),reversedAtoms:bigint('reversed_atoms',{mode:'bigint'}).notNull().default(BigInt(0)),updatedAt:timestamp('updated_at',{withTimezone:true}).notNull().defaultNow(),
},t=>[check('jcoin_accounts_conservation',sql`${t.balanceAtoms}>=0 and ${t.earnedAtoms}>=0 and ${t.reversedAtoms} between 0 and ${t.earnedAtoms} and ${t.balanceAtoms}=${t.earnedAtoms}-${t.reversedAtoms}`)]);
export const jcoinSourceEvents=pgTable('jcoin_source_events',{
  sourceId:uuid('source_id').primaryKey().references(()=>creditLedgerEntries.id),ownerId:uuid('owner_id').notNull().references(()=>users.id),outcome:text('outcome').notNull(),processedAt:timestamp('processed_at',{withTimezone:true}).notNull().defaultNow(),
},t=>[check('jcoin_source_events_outcome_check',sql`${t.outcome} in ('rewarded','refunded_before_reward','not_paid','unsupported_unit','outside_batch','zero_source','refund_applied')`)]);
export const jcoinLedgerEntries=pgTable('jcoin_ledger_entries',{
  id:uuid('id').primaryKey(),ownerId:uuid('owner_id').notNull().references(()=>jcoinAccounts.ownerId),batchId:uuid('batch_id').notNull().references(()=>jcoinBatches.id),sourceId:uuid('source_id').notNull().unique().references(()=>jcoinSourceEvents.sourceId),priorEntryId:uuid('prior_entry_id').unique().references(():AnyPgColumn=>jcoinLedgerEntries.id),kind:text('kind').notNull(),amountAtoms:bigint('amount_atoms',{mode:'bigint'}).notNull(),consumptionCredits:bigint('consumption_credits',{mode:'bigint'}),occurredAt:timestamp('occurred_at',{withTimezone:true}).notNull(),createdAt:timestamp('created_at',{withTimezone:true}).notNull().defaultNow(),
},t=>[index('jcoin_ledger_owner_time_idx').on(t.ownerId,t.occurredAt,t.id),check('jcoin_ledger_entry_shape',sql`(${t.kind}='mining_reward' and ${t.amountAtoms}>0 and ${t.priorEntryId} is null and ${t.consumptionCredits} is not null and ${t.consumptionCredits}>0) or (${t.kind}='refund_reversal' and ${t.amountAtoms}<0 and ${t.priorEntryId} is not null and ${t.consumptionCredits} is null)`)]);
export const jcoinAdministrativeActions=pgTable('jcoin_administrative_actions',{
  id:uuid('id').primaryKey(),actorOwnerId:uuid('actor_owner_id').notNull().references(()=>users.id),action:text('action').notNull(),idempotencyKey:text('idempotency_key').notNull(),operationHash:text('operation_hash').notNull(),result:jsonb('result').notNull(),createdAt:timestamp('created_at',{withTimezone:true}).notNull().defaultNow(),
},t=>[unique('jcoin_actions_idempotency').on(t.actorOwnerId,t.idempotencyKey),check('jcoin_actions_shape',sql`${t.action} in ('start','pause','resume','process') and length(${t.idempotencyKey}) between 8 and 200 and length(${t.operationHash})=64`)]);

export const feedbackTickets=pgTable('feedback_tickets',{
 id:uuid('id').primaryKey(),ownerId:uuid('owner_id').notNull().references(()=>users.id),category:text('category').notNull(),message:text('message').notNull(),status:text('status').notNull().default('open'),version:integer('version').notNull().default(1),requestKey:text('request_key').notNull(),fingerprint:text('fingerprint').notNull(),createdAt:timestamp('created_at',{withTimezone:true}).notNull().defaultNow(),updatedAt:timestamp('updated_at',{withTimezone:true}).notNull().defaultNow(),
},t=>[unique('feedback_tickets_owner_id_request_key_key').on(t.ownerId,t.requestKey),index('feedback_owner_time_idx').on(t.ownerId,t.createdAt,t.id),index('feedback_time_idx').on(t.createdAt,t.id),check('feedback_category_check',sql`${t.category} in ('generation','account','billing','assets','suggestion','other')`),check('feedback_status_check',sql`${t.status} in ('open','processing','resolved','closed')`),check('feedback_message_check',sql`char_length(${t.message}) between 1 and 4000`),check('feedback_version_check',sql`${t.version}>0`)]);
export const feedbackImages=pgTable('feedback_images',{
 ticketId:uuid('ticket_id').notNull().references(()=>feedbackTickets.id),position:integer('position').notNull(),objectKey:text('object_key').notNull().unique(),mimeType:text('mime_type').notNull(),byteSize:integer('byte_size').notNull(),
},t=>[primaryKey({columns:[t.ticketId,t.position]}),check('feedback_image_bounds',sql`${t.position} between 1 and 5 and ${t.byteSize} between 1 and 10485760`),check('feedback_image_mime',sql`${t.mimeType} in ('image/jpeg','image/png','image/webp')`)]);
