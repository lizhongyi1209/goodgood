...
test("GG-059 management navigation retains native history state without a document transition", () => {
  ...
  try {
    for (const route of [{ kind: "organizations" }, { kind: "admin", tab: "models" }, { kind: "admin", tab: "users" }, { kind: "create" }]) navigateWorkspace(route);
  } finally { if (original === undefined) delete globalThis.window; else globalThis.window = original; }
  assert.deepEqual(visits, ["/organizations", "/admin/models", "/admin/users", "/create"]);
  assert.deepEqual(events, Array(4).fill(WORKSPACE_NAVIGATION_EVENT));
});
...
test("GG-059 loading, signed-out, inactive, member and preview sessions never mount management", () => {
  ...
  for (const current of [undefined, null, session("user"), session("site_owner", "pending"), session("site_owner", "suspended"), session("site_owner", "active", true)]) {
    for (const activeTab of ["organizations", "models", "users"]) {
...
test("GG-059 active owner has three accessible links and embedded content with one management header", () => {
  for (const activeTab of ["organizations", "models", "users"]) {
    const html = render({ session: session(), activeTab });
    for (const href of ["/organizations", "/admin/models", "/admin/users"]) assert.ok(html.includes(`href="${href}"`));
    ...
    assert.match(html, activeTab === "organizations" ? /enterprise-private-content/ : activeTab === "models" ? /正在读取模型/ : /搜索邮箱[\s\S]*最近操作记录/);
  }
});

test("GG-059 admin entry points mount the same shell and management changes refresh shared data", async () => {
  for (const tab of ["models", "users"]) assert.match(await readFile(new URL(`../app/admin/${tab}/page.tsx`, import.meta.url), "utf8"), /export \{ default \} from "@\/app\/page"/);
