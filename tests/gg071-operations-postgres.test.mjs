...
test('GG-071 isolated SQL calendar, exact unit conversion, enterprise attribution and ledger lookup',{skip:!enabled},async()=>{
  ...
  try {
    ...
    await client.query(`CREATE TABLE users(id uuid,email text,created_at timestamptz);
      ...
      CREATE TABLE generation_jobs(id uuid,batch_id uuid,creator_owner_id uuid,workspace_id uuid,submitted_at timestamptz,completed_at timestamptz,state text,error_code text);
    ...
    await client.query(`INSERT INTO generation_jobs VALUES
      ($1,$5,$9,$10,$12,$11,'succeeded',NULL),
      ($2,$6,$9,$13,$11,$11,'succeeded',NULL),
      ($3,$7,$9,$10,$11,$11,'failed','GENERATION_FAILED'),
      ($4,$8,$9,$10,$14,NULL,'queued',NULL)`,[id(12),id(13),id(14),id(15),id(8),id(9),id(10),id(11),id(1),id(3),start,before,id(4),end]);
    ...
    assert.deepEqual(dashboard.days[0],{day:'2026-09-14',jobs:'2',creators:'1',succeeded:'2',failed:'1',cancelled:'0',users:'1',settled:'50',refunded:'10',released:'9007199254740993'});
