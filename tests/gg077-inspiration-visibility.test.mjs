...
test('GG077 cards expose view/use statistics without depending on opening details',()=>{
 ...
 assert.match(html,/aria-label="查看次数：42"/);assert.match(html,/aria-label="使用次数：7"/);assert.doesNotMatch(html,/42 查看|7 使用/);assert.match(html,/aria-label="点赞：案例"/);assert.match(html,/case-like/);
});
